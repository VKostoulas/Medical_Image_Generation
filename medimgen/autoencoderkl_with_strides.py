...
class Upsample(nn.Module):
    ...
    def __init__(self, spatial_dims: int, in_channels: int, use_convtranspose: bool, stride=2, kernel_size=4, padding=1) -> None:
        super().__init__()
...
class Downsample(nn.Module):
    ...
    def __init__(self, spatial_dims: int, in_channels: int, stride=2, kernel_size=4, padding=1) -> None:
        super().__init__()
            ...
            if not is_final_block:
                ...
                blocks.append(
                    Upsample(spatial_dims=spatial_dims, in_channels=block_in_ch, use_convtranspose=use_convtranspose,
                             stride=temp_stride, kernel_size=temp_kernel_size, padding=temp_padding)
                )
...
class AutoencoderKL(nn.Module):
    ...
    def __init__(
        ...
        use_convtranspose: bool = False,
        downsample_parameters: Sequence[Sequence[int, int, int], ...]
                               | Sequence[int, int, int] = ((2, 4, 1), (2, 4, 1), (2, 4, 1)),
        upsample_parameters: Sequence[Sequence[int, int, int], ...]
                             | Sequence[int, int, int] = ((2, 4, 1), (2, 4, 1), (2, 4, 1)),
    ) -> None:
        super().__init__()
        ...
        self.encoder = Encoder(
            ...
            use_flash_attention=use_flash_attention,
            strides=[item[0] for item in downsample_parameters],
            kernel_sizes=[item[1] for item in downsample_parameters],
            paddings=[item[2] for item in downsample_parameters]
        )
        self.decoder = Decoder(
            ...
            strides=[item[0] for item in upsample_parameters],
            kernel_sizes=[item[1] for item in upsample_parameters],
            paddings=[item[2] for item in upsample_parameters]
        )
