...
class Downsample(nn.Module):
    ...
    def __init__(
        self, spatial_dims: int, num_channels: int, use_conv: bool, out_channels: int | None = None,
            stride=2, kernel_size=4, padding=1) -> None:
        super().__init__()
        self.num_channels = num_channels
        ...
        if use_conv:
            ...
            self.op = Pool[Pool.AVG, spatial_dims](kernel_size=kernel_size, stride=stride)
...
class ResnetBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.up:
            self.upsample = Upsample(spatial_dims, in_channels, use_conv=False, stride=stride, padding=padding)
        elif down:
            self.downsample = Downsample(spatial_dims, in_channels, use_conv=False, kernel_size=kernel_size, stride=stride, padding=padding)
...
class DiffusionModelUNet(nn.Module):
    ...
    def __init__(
        ...
        use_flash_attention: bool = False,
        dropout_cattn: float = 0.0,
        strides: Sequence[Sequence[int, int, int], ...]
                 | Sequence[int, int, int] = ((2, 2, 2), (2, 2, 2), (2, 2, 2)),
        kernel_sizes: Sequence[Sequence[int, int, int], ...]
                      | Sequence[int, int, int] = ((4, 4, 4), (4, 4, 4), (4, 4, 4)),
        paddings: Sequence[Sequence[int, int, int], ...] = (1, 1, 1)
    ) -> None:
        super().__init__()
        ...
        for i in range(len(reversed_block_out_channels)):
            ...
            up_block = get_up_block(
                ...
                use_flash_attention=use_flash_attention,
                dropout_cattn=dropout_cattn,
                stride=list(reversed(strides))[i] if not is_final_block else None,
                kernel_size=list(reversed(kernel_sizes))[i] if not is_final_block else None,
                padding=list(reversed(paddings))[i] if not is_final_block else None
            )
